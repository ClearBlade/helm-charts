...
`global.awsHelmRoleArn` is also used for the streaming replica's static egress IP - if `cb-postgres.streamingReplica.staticEgressIP.enabled` is set, the same role additionally needs `ec2:DescribeAddresses`, `ec2:AssociateAddress`, and `ec2:DescribeInstances`, trusted for the `clearblade-streaming-egress` service account as well. The Elastic IP itself is pre-provisioned once with `replica-patches/allocate-egress-ip.sh` (not allocated by the chart, so it isn't leaked on `helm uninstall`) - its allocation ID goes in `cb-postgres.streamingReplica.staticEgressIP.allocationId`. Both `replica-patches/create-helm-role.sh` and `replica-patches/allocate-egress-ip.sh` are run automatically by `replica-patches/configure_streaming.sh` when setting up a streaming replica.
