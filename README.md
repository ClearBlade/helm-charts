...
`replica-patches/create-helm-role.sh` is run automatically by `replica-patches/configure_streaming.sh` when setting up a streaming replica, but can also be run standalone per account/namespace.
